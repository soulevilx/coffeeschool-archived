  ...
  public function ccnGetUserDetails($userId) {
    ...
    if ($DB->record_exists('user', array('id' => $userId))) {
      ...
      if($moreUserData->deleted !== '1'){
        ...
        $userIcq = $userData->icq ?? null;
        $userSkype = $userData->skype ?? null;
        $userYahoo = $userData->yahoo ?? null;
        $userAim = $userData->aim ?? null;
        $userMsn = $userData->msn ?? null;
        $userPhone1 = $userData->phone1 ?? null;
        $userPhone2 = $userData->phone2 ?? null;
        /* ccnTkt: 2962
