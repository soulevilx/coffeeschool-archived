    ...
    public function get_content()
    {
        ...
        if (is_enrolled($context, $USER, '', true)) {
          ...
          if(!empty($this->content->price) || !empty($this->content->price_title) || !empty($this->content->full_price)){
            $this->content->text .='<div class="price"><span>'. $this->content->price_title .'</span> '. $this->content->price .' <small>'. $this->content->full_price ?? NULL .'</small></div>';
          }
          ...
          if ($data->items > 0) {
            ...
            for ($i = 1; $i <= $data->items; $i++) {
              ...
              $this->content->text .='<li><div class="ccn-course-details-item"><span class="'.format_text($item_icon1, FORMAT_HTML, array('filter' => true)).'"></span> '.format_text($item_title, FORMAT_HTML, array('filter' => true)).'</div></li>';
            }
